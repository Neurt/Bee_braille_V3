...
#                     print("masuk if check its page sign")
                    print(linesInFile)
                else:  # TextContent
#                     print("masuk else goblok")
                    linesInPage += 1
                    linesInEachPage += 1
    except UnicodeDecodeError:
#         print ("DBEUG:",error)
        print("please convert to correct brf")
        with open(File, "r") as f:
            print ("masuk try cekfiles epub")
            
            convert_epub(File, path)
...
def downloadBook(filename):
    global mode, splited, isBookAvailable, pageIndex, start_time
    splited.clear()
...
#             response = req.patch("http://" + ip + "/book/returnBook", params = {"accessToken": accessToken, "refreshToken": refreshToken, "isbn": filename})
            ...
            if len(response.content) == 0:
                ...
                end_time_downloadButton = time.time()
                execution_time_ms_downloadButton = (end_time_downloadButton - start_time) * 1000
                print(f"Code execution time downloadButton: {execution_time_ms_downloadButton} ms")
...
def split_lines_with_separator(text, lines_per_page):
    lines = text.split('\n')
    pages = []
...
def get_separator_label(page_number):
    page_number_str = str(page_number)
    label = ""
    for char in page_number_str:
        if char == '0':
            letter = "#J"
        else:
            letter = '#' + chr(64 + int(char))
        label = label + letter
    return f'{label}'
...
def write_pages_to_file(pages, filename):
    with open(filename, 'w', encoding='utf-8') as file:
        for i, page in enumerate(pages, start=1):
            separator_label = get_separator_label(i)
            file.write(separator_label + '\n')
            file.write('\n'.join(page) + '\n')

#1234567891011123
#3173080406010003 NIK
def convert_epub(epub_path, destination_path):
    file_name = os.path.basename(epub_path)
    epub_filename = os.path.splitext(file_name)[0]

    converted_raw = epub2txt(epub_path, clean= False)
    converted_upper = ''.join(char.upper() if char.islower() else char for char in converted_raw) # convert to upper case
    converted_lines = converted_upper.split('\n')
    converted_wrap_lines = [textwrap.fill(linee, width = 70) for linee in converted_lines]
    
#     converted_wrap_lines = []
#     for linee in converted_lines:
#         converted_wrap_line = ''
#         for i in range (0, len(linee), 40):
#             converted_wrap_line += linee[i:i+40] + '\n'
#         converted_wrap_lines.append(converted_wrap_line)
    
    result = '\n'.join(converted_lines)

    lines_per_page = 27
    pages = split_lines_with_separator(result, lines_per_page)
    write_pages_to_file(pages, destination_path + '/' + epub_filename + '.brf')

def searching_usb_book(dir):
    global USBpath, choosenBook, book_files
    books_folder = os.path.join(dir, "epub_folder/")
    
    if not os.path.exists(books_folder):
        print("No 'epub_folder' folder found.")
        return
    
    # create new folder to accomodate the converted epub(s)
    brf_path = os.path.join(books_folder, "ebraille_converted_epub/")
    if not os.path.exists(brf_path):
        os.makedirs(brf_path)
    
    epub_files = [f for f in os.listdir(books_folder) if f.endswith(".epub")]
    
    # convert all the detected epub(s)
    for epub_file in epub_files:
        epub_file_path = os.path.join(books_folder, epub_file)
        convert_epub(epub_file_path, brf_path)
    
    book_files = [f for f in os.listdir(brf_path) if f.endswith(".brf")]
...
#                     print("halo")
                    ...
                    buttonClicked = rw.buttonPressed()
#                     rw.writeBraille("MIKE")
#                     print("MIKE shudvebeen written")
        ...
        #                 print(loadedData)c
                    ...
                    # MAINPAGE
                        ...
                        if buttonClicked == downloadButton:
                            ...
                            end_time_helpButton = time.time()
                            execution_time_ms_helpButton = (end_time_helpButton - start_time) * 1000000
                            print(f"Code execution time helpButton: {execution_time_ms_helpButton} ms")
                            callHelp()
                        # usb button = 10
                        elif buttonClicked == usbButton or buttonClicked == escButton: #kembali ke mode tanpa USB
                            end_time_usbButton = time.time()
                            execution_time_ms_usbButton = (end_time_usbButton - start_time) * 1000000
                            print(f"Code execution time usbButton: {execution_time_ms_usbButton} ms")
